...
const generateJWTToken = (user) => {
    try {
        ...
        const token = jwt.sign(jwtPayload, jwtSecret || 'dev_secret_fallback_12345', { expiresIn: '7d' });
...
const deleteUser = async (req, res) => {
    try {
        ...
        // Vérifier que ce n'est pas le dernier admin (sécurité)
        if (user.role === 'admin') {
            const adminCount = await User.countDocuments({ role: 'admin' });
            if (adminCount <= 1) {
                console.log('⚠️ Tentative de suppression du dernier admin bloquée');
                return res.status(400).json({
                    success: false,
        ...
        console.log(`✅ Utilisateur supprimé: ${deletedUserInfo.email}`);
        res.status(200).json({
            success: true,
...
const autoApproveAllPending = async (req, res) => {
    try {
        const result = await User.updateMany({ status: 'pending' }, {
            $set: {
                status: 'active',
                approvedAt: new Date(),
                approvedBy: null
